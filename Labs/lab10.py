...
class Game:
    ...
    def play(self):
        ...
        while not self.game_over:
            if self.turn in a:
                Player.choose(self.p1, self.p2)(self.p2)
                self.turn += 1
            if self.turn in b:
                Player.choose(self.p2, self.p1)(self.p1)
                self.turn += 1
        return self.winner
    ...
    @property
    def winner(self):
        if self.turn < 10 and max(self.p1.votes, self.p2.votes) >= 50:
            if max(self.p1.votes, self.p2.votes) == self.p1.votes:
                return self.p1.votes
            else:
                return self.p2.votes
        else:
...
class Beater(QuidditchPlayer):
    ...
    def energy(self, time):
        ...
        """
        try:
            self.base_energy -= self.base_energy/time
            return self.base_energy
...
class Chaser(QuidditchPlayer):
    ...
    def energy(self, time):
        ...
        """
        try: 
            self.base_energy -= (self.goals)*(20)
            if time % 9 ==0:
                self.base_energy -= 20 * 0.1
            return self.base_energy
        except ZeroDivisionError:
            print("You can't divide by zero!")
...
class Keeper(QuidditchPlayer):
    ...
    def energy(self, time):
        ...
        try: 
            if time >= 30:
                self.base_energy -= (0.8)*(50)*(time//15)
            return self.base_energy
        except ZeroDivisionError:
            print("You can't divide by zero!")
