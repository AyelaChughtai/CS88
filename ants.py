...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # # BEGIN EC
        if self.direction_forwards == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        position = self.place
        index = 0
        while position != beehive and position.entrance != None:
            if len(position.bees) > 0 and index >= self.min_range and index <= self.max_range:
   ...
   # Change to True to view in the GUI
    ...
    def action(self, colony):
        ...
        if self.imposter == True:
            ...
            while position != None:
                if position.ant and position.ant not in self.list_of_ants:
                    position.ant.damage = position.ant.damage * 2
                    self.list_of_ants = self.list_of_ants + [position.ant]
                if position.ant and position.ant.is_container and position.ant.contained_ant and position.ant.contained_ant not in self.list_of_ants:
                    position.ant.contained_ant.damage = position.ant.contained_ant.damage * 2
                    self.list_of_ants = self.list_of_ants + [position.ant.contained_ant]
...
def make_scare(action, bee):
    ...
    def scare(colony):
        bee.scared = True
        bee.direction_forwards = False
        action(colony)
        bee.direction_forwards = True
    return scare
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    action_with_effect = effect(bee.action, bee)
    action_without_effect = bee.action
    def apply(colony):
        nonlocal duration
        if duration > 0:
            action_with_effect(colony)
            duration = duration - 1
        else:
            action_without_effect(colony)
    bee.action = apply
