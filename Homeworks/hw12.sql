...
CREATE TABLE long as
  select name, milliseconds FROM tracks WHERE milliseconds > 360000;


-- Question 4
CREATE TABLE largest as
  select name, milliseconds FROM tracks ORDER BY bytes DESC LIMIT 1;


-- Question 5
CREATE TABLE long_album as
  select albums.title FROM albums, tracks WHERE tracks.milliseconds > 360000 and albums.albumID = tracks.albumId;
...
CREATE TABLE album_count as
  select artists.name, count(*) FROM artists, albums WHERE artists.artistID = albums.artistID group by artists.name;


-- Question 8
CREATE TABLE busiest_artists as
  select artists.name, count(*) FROM artists, tracks, albums WHERE tracks.albumID = albums.albumId AND artists.artistID = albums.artistID GROUP BY artists.name;
